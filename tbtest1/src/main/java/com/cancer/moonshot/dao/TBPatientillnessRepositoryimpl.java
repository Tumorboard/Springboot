package com.cancer.moonshot.dao;

import java.util.ArrayList;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.cancer.moonshot.entity.PatientillnessEntity;
import com.cancer.moonshot.repository.TBPatientillnessRepository;
import com.google.gson.Gson;
@Component
public class TBPatientillnessRepositoryimpl implements  TBPatientillnessDAO{

	@Autowired
	TBPatientillnessRepository PatientillnessRepo;
	
	@Autowired
	Gson gson;
	
	public List<PatientillnessEntity> loadTbPatientillnessDetails(int id){
		List<PatientillnessEntity> summaryDetails = PatientillnessRepo.findBypatientIdFk(id);
		List<PatientillnessEntity> tbDetails = new ArrayList<>();
		
		summaryDetails.stream().forEach(tumorBoardDetails ->{
			//if needed we can do operations here and type cast details to final pojo 
			tbDetails.add(gson.fromJson(gson.toJson(tumorBoardDetails),PatientillnessEntity.class));
			
		});
		
		return tbDetails;
	}
	

}