package com.cancer.moonshot.dao;

import java.util.ArrayList;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

import com.cancer.moonshot.entity.PatientillnessEntity;
import com.cancer.moonshot.entity.PhysicalexaminationEntity;
import com.cancer.moonshot.repository.TBPatientillnessRepository;
import com.cancer.moonshot.repository.TBPhysicalexaminationRepository;
import com.google.gson.Gson;
@Component
public class TBPhysicalexaminationRepositoryimpl implements  TBPhysicalexaminationDAO{

	@Autowired
	TBPhysicalexaminationRepository PhysicalexaminationRepo;
	
	@Autowired
	Gson gson;
	
	public List<PhysicalexaminationEntity> loadTbPhysicalexaminationDetails(int id){
		List<PhysicalexaminationEntity> summaryDetails = PhysicalexaminationRepo.findBypatientIdFk(id);
		List<PhysicalexaminationEntity> tbDetails = new ArrayList<>();
		
		summaryDetails.stream().forEach(tumorBoardDetails ->{
			//if needed we can do operations here and type cast details to final pojo 
			tbDetails.add(gson.fromJson(gson.toJson(tumorBoardDetails),PhysicalexaminationEntity.class));
			
		});
		
		return tbDetails;
	}
	

}